...
async function supabaseRequest<T>(path: string, init: RequestInit = {}, env: Env): Promise<T> {
...
async function authenticate(request: Request, env: Env): Promise<string | null> {
  if (env.NEXOVA_ENVIRONMENT === 'development') return 'development-user';
...
async function isTenantMember(userId: string, tenantId: string, env: Env): Promise<boolean> {
  if (env.NEXOVA_ENVIRONMENT === 'development') return true;
  const rows = await supabaseRequest<Array<{ tenant_id: string }>>(
    `memberships?user_id=eq.${encodeURIComponent(userId)}&tenant_id=eq.${encodeURIComponent(tenantId)}&select=tenant_id`,
    {},
    env,
  );
...
async function midtransRequest<T>(path: string, init: RequestInit = {}, env: Env): Promise<T> {
  ...
  const result = (await response.json()) as T & { status_code?: string; status_message?: string };
  if (!response.ok) {
...
function toApiOrder(order: SupabaseOrder): Record<string, unknown> {
  return {
    id: order.id,
...
async function findPersistentOrder(id: string, env: Env): Promise<Record<string, unknown> | null> {
  const rows = await supabaseRequest<SupabaseOrder[]>(
...
async function findPersistentOrderByIdempotency(
  tenantId: string,
  idempotencyKey: string,
  env: Env,
): Promise<SupabaseOrder | null> {
  const rows = await supabaseRequest<SupabaseOrder[]>(
    `orders?tenant_id=eq.${encodeURIComponent(tenantId)}&idempotency_key=eq.${encodeURIComponent(idempotencyKey)}&select=*`,
    {},
    env,
  );
  return rows[0] ?? null;
...
async function route(request: Request, env: Env): Promise<Response> {
  ...
  if (method === 'POST' && path === '/api/v1/payments') {
    ...
    const result = await midtransRequest<{
  ...
  if (method === 'GET' && paymentMatch) {
    ...
    const result = await midtransRequest<{ transaction_status?: string; order_id?: string }>(
      `/v2/${encodeURIComponent(paymentMatch[1])}/status`,
      {},
      env,
    );
    return send(200, {
  ...
  if (method === 'POST' && path === '/api/v1/orders') {
    ...
    if (usesConfiguredSupabaseStorage) {
      ...
      const rows = await supabaseRequest<SupabaseOrder[]>('orders', {
        method: 'POST',
