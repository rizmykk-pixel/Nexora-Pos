...
const orderIdempotency = new Map<string, { payload: string; order: Record<string, unknown> }>();
...
if (runtimeEnvironment !== 'development' && !usesConfiguredSupabaseStorage) {
  throw new Error(
    'Persistent Supabase storage is required outside development. Set NEXOVA_STORAGE_MODE=supabase and provide SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY.'
  );
}
...
async function authenticate(request: Request): Promise<string | null> {
  if (runtimeEnvironment === 'development') return 'development-user';
...
async function isTenantMember(userId: string, tenantId: string): Promise<boolean> {
  if (runtimeEnvironment === 'development') return true;
...
async function findPersistentOrder(id: string): Promise<Record<string, unknown> | null> {
